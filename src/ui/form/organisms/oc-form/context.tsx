...
import { OcFormContextProviderProps, OcFormContextProps } from './types';
...
export const OcFormContextProvider: React.FC<OcFormContextProviderProps> = (
	...
	const onRemoveDynamicField = React.useCallback((event: React.SyntheticEvent<Dataset>) => {
		const elementPath = event.currentTarget.dataset.path;
		...
		if (isFirstLevelDeep) {
			...
			next = update(next, path, (fields: FormikField[]) => {
				const removeChildrenFields = fields.filter(
					(f) => f.type === FIELD_TYPE.DYNAMIC_FIELD_ARRAY
				).length === 1;
